...
// Changed from edge to nodejs runtime to support MySQL queries
...
export async function GET(req: Request) {
  ...
  |> last()`
    ...
    // Try to get data from InfluxDB, but continue even if it fails
    let raw = ''
    try {
      ...
      if (res.ok) {
        ...
        console.warn('InfluxDB query failed:', res.status, text)
      }
    } catch (influxErr) {
      console.warn('InfluxDB connection failed:', influxErr)
      // Continue to MySQL query even if InfluxDB fails
    }
  ...
  parsed.location = (locMatch && locMatch[1]) || (siteMatch && siteMatch[1]) || (locTagMatch && locTagMatch[1]) || undefined
    ...
    // Query MySQL to get series_no from devices table
    try {
      const devices = await queryKsave(
        'SELECT series_no, location, deviceName FROM devices WHERE ksaveID = ? OR deviceName = ? LIMIT 1',
        [id, id]
      )
      if (devices && devices.length > 0) {
        const device = devices[0]
        // Override with database values (database is the source of truth)
        if (device.series_no) parsed.seriesNo = device.series_no
        if (device.location && !parsed.location) parsed.location = device.location
        if (device.deviceName && !parsed.seriesName) parsed.seriesName = device.deviceName
      }
    } catch (mysqlErr) {
      // MySQL query failed, continue with InfluxDB data only
      console.error('MySQL query failed:', mysqlErr)
    }
