...
export async function GET(req: Request) {
  try {
    ...
    // If Influx is not reachable, provide sample fallback data so the UI can render
    // ⚠️ NOTE: This is fallback data for when InfluxDB is unavailable
    console.warn('InfluxDB unavailable, using fallback data:', String(err))
    const sample = [
