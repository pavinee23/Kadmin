    ...
    ;(async () => {
      try {
        ...
        if (res.ok && j && j.success) {
          ...
          router.push('/Thailand/Admin-Login/receipt/list')
        } else {
        ...
        <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 9999 }}>
          <div style={{ minWidth: 280, maxWidth: '96vw', background: '#fff', padding: 12, borderRadius: 10, boxShadow: '0 6px 18px rgba(0,0,0,0.12)' }}>
              ...
              {savedReceipts.map((r: any) => (
                <div key={r.receiptID} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderTop: '1px solid #f1f5f9' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <a href={`/Thailand/Admin-Login/receipt/print?receiptID=${r.receiptID}`} target="_blank" rel="noreferrer">
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10 }}>
              <a href="/Thailand/Admin-Login/receipt/list"><button className={`${styles.btn} ${styles.btnSecondary}`}>{L('View All', 'ดูทั้งหมด')}</button></a>
            </div>
          </div>
        </div>
      )}
